...
<html>
    ...
    <div class="tab" >
        ...
        <button class="tablinks" onclick="openfile(event, 'supervise')">الفعالية التي أشرف عليها</button>
    ...
    <div id="PersonalInfo" class="tabcontent">
        ...
        <?php
        // $_SESSION["username"]="basmah555"; //0000
        //$username = $_SESSION["username"];
//$f = $_POST["username"];
//echo "$f ".$f;
//echo "<form method='post' name='username' action='leaderprofile.php'>";
...
// Connect to MySQL
    ...
    <!--  صفحة الفعاليات التي اشرف عليها -->		

    <div id="supervise" class="tabcontent">
        <h3>الفعاليات التي اشرف عليها</h3>
        <center>
            <h4>اسم الفعالية</h4>
            <p>مسمى القسم المشرف علية</p>
             <!--<table border=2 width=300>
                    <tr>
                    <td>مجموع الساعات</td><td>وقت الخروج</td><td>وقت الدخول</td><td>الإسم</td>-->
            <?php
            $query = "
select volunteer.FirstName, volunteer.MiddleName, volunteer.LastName, volunteerparticipateonevent.StartingHour, volunteerparticipateonevent.EndingHour , SUM(StartingHour+EndingHour) from volunteer, volunteerparticipateonevent, leaderleadvolunteers 
WHERE leaderleadvolunteers.Volunteer_ID = volunteerparticipateonevent.Volunteer_ID AND  volunteerparticipateonevent.Volunteer_ID = volunteer.VolunteerID AND
leaderleadvolunteers.LeaderID = (SELECT volunteer.VolunteerID from volunteer where volunteer.VolunteerUsername='$username')";
    ...
    <!-- صفحة الفعاليات التي شاركت فيها  -->		

    <div id="Participation" class="tabcontent">
        <h3>الفعاليات التي شاركت فيها</h3>


        <!--<td>الشهادة</td><td>عدد ساعات مشاركتي</td><td>إسم الفعالية</td>
        -->
        <?php
